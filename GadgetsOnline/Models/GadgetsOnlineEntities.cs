...
namespace GadgetsOnline.Models
{
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        public GadgetsOnlineEntities(string dbConn)
        {
            _dbConn = dbConn;
        }
        ...
        { /* Use Configuration from static ConfigurationManager to access connection string in appsettings.json. Example below: optionsBuilder.UseSqlServer(ConfigurationManager.Configuration.GetConnectionString("CONNECTIONSTRINGNAME")); */
            ...
            {
                optionsBuilder.UseNpgsql(_dbConn).UseUpperCaseNamingConvention();
            }
            else
            {
        ...
        {
            modelBuilder.HasDefaultSchema("gadgetsonline_dbo");
            ...
            foreach (var entity in modelBuilder.Model.GetEntityTypes())
            {
                ...
                {
                    property.SetColumnName(ConvertToLowercase(property.GetColumnName()));
                }

                foreach (var key in entity.GetKeys())
                {
                    key.SetName(ConvertToLowercase(key.GetName()));
                }

                foreach (var key in entity.GetForeignKeys())
                {
                    key.SetConstraintName(ConvertToLowercase(key.GetConstraintName()));
                }
